...
CREATE TABLE Transactions (
 ...
 amount DECIMAL(12,2),
 transaction_type NVARCHAR(10) CHECK (transaction_type IN ('Expense', 'Revenue')), category NVARCHAR(50),
...
transaction_type, category) VALUES
...
SELECT invoice_id, due_date, status, c.name, c.contact_email FROM Invoices AS i 
JOIN Clients AS c ON i.client_id = c.client_id
WHERE due_date < GetDate();

-- Find all payments made by bank transfer.

SELECT * FROM Payments WHERE payment_method = 'Bank Transfer';

-- Show the total revenue received from paid invoices

SELECT SUM(amount - vat - discount) AS TotalRevenue FROM Invoices WHERE status = 'Paid';

-- Count the number of invoices per status (Pending, Paid, Overdue).

SELECT COUNT(status) AS [Count], status FROM Invoices GROUP BY status

-- 04. Advanced Queries

-- Retrieve all transactions for a specific supplier (e.g., supplier_id = 3).

SELECT * FROM Transactions WHERE supplier_id = 3;
...
SELECT SUM(amount) AS TotalExpenses FROM Transactions 
WHERE transaction_type = 'Expense' AND 
transaction_date >= '2024-01-01' AND transaction_date < '2024-02-01'; 

-- Find the highest and lowest invoice amounts.

SELECT MAX(amount) AS HighestAmount, MIN(amount) AS LowestAmount FROM Invoices;

-- 05. Reports

-- Create a view showing invoices with their corresponding payments

CREATE OR ALTER VIEW InvoicesWithPayments AS
SELECT 
    i.invoice_id,
    i.client_id,
    i.issue_date,
...
SELECT * FROM dbo.InvoicesWithPayments;

-- Create a report showing monthly revenue for the year 2024.

SELECT 
    MONTH(issue_date) AS [Month],
    SUM(amount - vat - discount) AS TotalRevenue
FROM Invoices
WHERE YEAR(issue_date) = 2024 AND status = 'Paid'
GROUP BY MONTH(issue_date)
ORDER BY [Month];

-- Find the total amount paid per client.

SELECT SUM(p.amount) AS TotalAmount, name
FROM Payments AS p
JOIN Invoices AS i ON p.invoice_id = i.invoice_id
JOIN Clients AS c ON i.client_id = c.client_id
GROUP BY c.name;

-- List all invoices along with their VAT amounts.

SELECT invoice_id, vat FROM Invoices;

-- Find the total payments received per payment method.

SELECT SUM(amount) AS TotalPayments, payment_method FROM Payments 
GROUP BY payment_method

-- Write a stored procedure to fetch all invoices for a given client ID.

CREATE PROCEDURE dbo.GetClientInvoices (@clientID INT)
AS
BEGIN
...
EXEC GetClientInvoices 1;

-- Find the percentage of overdue invoices relative to total invoices.

DECLARE @overdueInvoices INT = (SELECT COUNT(*) FROM Invoices WHERE status = 'Overdue');
SELECT @overdueInvoices * 100.0 / COUNT(*) AS [Overdue Invoices Percentage] FROM Invoices;

-- SELECT COUNT(CASE WHEN [status] = 'Overdue' THEN 1 END) * 100.0 / COUNT(*) FROM Invoices;

-- Generate a report showing the top 3 clients who have paid the most

SELECT TOP 3 SUM(p.amount) AS TotalPayments, c.name FROM Payments AS p
JOIN Invoices AS i ON p.invoice_id = i.invoice_id
